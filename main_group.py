...
async def album_handler(messages: List[types.Message]):
    if await check_ban(messages[0].from_id):
        ...
        try:
            ...
            if 'bot was blocked by the user' in str(e):
                await messages[0].answer('Пользователь #id' + str(messages[0].reply_to_message.text.split('#id')[1].split(',')[0]) + ' заблокировал бота')
            else:
                try:
...
async def predict_answer_media(message: types.Message):
    try:
        ...
        if 'bot was blocked by the user' in str(e):
            await message.answer('Пользователь #id' + str(message.reply_to_message.text.split('#id')[1].split(',')[0]) + ' заблокировал бота')
        else:
            try:
